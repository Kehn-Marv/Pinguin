...
class OllamaStarter {
  ...
  private instanceType: OllamaInstanceType | null;
  ...
  /**
   * Return the host of the Ollama server that is running. If the server is not running, return default host
   * @returns The host of the Ollama server
   */
  public getHost(): string {
    if (!this.host) {
      // Return default host instead of throwing error
      // This allows graceful initialization even if Ollama isn't fully ready yet
      const defaultHost = process.env.OLLAMA_HOST || "http://localhost:11434";
      // Use debug level to reduce noise during embedding batches
      logger.debug(`Ollama host not set yet, returning default: ${defaultHost}`);
      return defaultHost;
    }
    return this.host;
  }
  ...
  public async start(): Promise<OllamaInstanceType> {
    if (this.isRunning && this.instanceType) {
    ...
    try {
      ...
      if (!this.instanceType) {
        throw new Error("Instance type not set after starting");
      }
      return this.instanceType;
    } catch (error) {
      ...
      this.childProcess = exec("ollama serve", { env }, (error) => {
        ...
        this.ping(host, 5).then((isRunning) => {
          if (isRunning) {
            ...
            const processMonitor = getProcessMonitor();
            if (this.childProcess) {
              processMonitor.registerProcess("ollama", this.childProcess, {
                name: "ollama",
