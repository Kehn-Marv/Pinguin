...
class DocumentsDB {
   ...
   * @param chatId The id of the chat to which the document belongs
   * @returns the newly created document
   */
  public createDocument(documentPath: string, courseId: string, chatId: string): Doc {
  ...
  public getDocumentsByChatId(chatId: string): Doc[] {
    return this.documents.filter((doc) => doc.chatId === chatId);
  }
  ...
  public deleteDocumentsByChatId(chatId: string): void {
    const documents = this.getDocumentsByChatId(chatId);
    for (const document of documents) {
      this.deleteDocument(document.id);
    }
  }
