...
const model = {
  ...
  getSelectedEmbedding: async (): Promise<ModelID | null> =>
    ipcRenderer.invoke("model:getSelectedEmbedding"),
  getSelectedLLM: async (): Promise<ModelID | null> =>
    ipcRenderer.invoke("model:getSelectedLLM"),
  subscribeToSelectedEmbedding: (
    listener: (_event: IpcRendererEvent, modelId: string | null) => void
  ) => {
  ...
  refreshList: async (): Promise<void> =>
    ipcRenderer.invoke("model:refreshList"),
};
