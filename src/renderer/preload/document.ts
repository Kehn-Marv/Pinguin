...
const document = {
  ...
  getAllByChat: async (chatId: string): Promise<Doc[]> => {
    return ipcRenderer.invoke("document:getAllByChat", chatId);
  },
  import: (courseId: string, chatId: string): void => {
    ipcRenderer.invoke("document:import", courseId, chatId);
  },
